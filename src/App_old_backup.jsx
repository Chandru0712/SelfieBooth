...
import { APP_CONFIG, FILTER_PRESETS, CAMERA_CONFIG, ERROR_MESSAGES } from './constants';
...
const frameDimensionsCache = new Map();
...
function App() {
  ...
  const [useIPCam, _setUseIPCam] = useState(true);
  ...
  // Detect aspect ratio when selectedFrame changes
  useEffect(() => {
    ...
    // Check cache first
    if (frameDimensionsCache.has(frame.image)) {
      const ratio = frameDimensionsCache.get(frame.image);
      setAspectRatio(ratio);
      return;
    }

    // If not cached, detect asynchronously (non-blocking)
    const img = new Image();
    img.onload = () => {
      const ratio = img.naturalWidth > img.naturalHeight ? '16 / 9' : '9 / 16';
      frameDimensionsCache.set(frame.image, ratio);
      setAspectRatio(ratio);
    };
    img.src = frame.image;
  ...
  const handleMouseMove = (e) => {
    ...
    const walk = (x - startX) * CAMERA_CONFIG.SCROLL_SPEED_MULTIPLIER; // Scroll speed
  ...
  useEffect(() => {
    if (scrollRef.current && selectedFrame && !isMouseDown) {
      ...
      if (activeItem) {
        ...
        if (Math.abs(containerCenter - itemCenter) > APP_CONFIG.SCROLL_THRESHOLD) {
          isProgrammaticScroll.current = true;
          activeItem.scrollIntoView({ behavior: APP_CONFIG.SMOOTH_SCROLL_BEHAVIOR, inline: 'center' });
          
          if (scrollTimeout.current) clearTimeout(scrollTimeout.current);
          scrollTimeout.current = setTimeout(() => {
            isProgrammaticScroll.current = false;
          }, APP_CONFIG.SCROLL_TIMEOUT_MS);
        }
      }
    }
  }, [selectedFrame, isMouseDown]);
  ...
  const [filter, _setFilter] = useState('none');
  const [showFlash, setShowFlash] = useState(false);
  const [countdown, setCountdown] = useState(null);

  // Use constants for filter presets
  ...
  const takePhoto = () => {
    ...
    const timer = setInterval(() => {
      ...
      if (count > 0) {
        ...
        setTimeout(() => setShowFlash(false), APP_CONFIG.FLASH_DURATION_MS);
      }
    }, 1000);
  };
        ...
        {!isActive ? (
          ...
          <>
            ...
            <div className="booth-container">
              <div 
                className={`viewfinder-wrapper ${aspectRatio === '9 / 16' ? 'portrait' : 'landscape'}`}
                style={{ aspectRatio }}
              >
                ...
                <div className="action-section">
                  <div className="action-row">
                    <div className="timer-container" ref={timerContainerRef}>
                      {showTimerMenu && (
                        <div className="timer-dropdown glass-premium">
                          {APP_CONFIG.TIMER_OPTIONS.map(time => (
                            <button 
                              key={time} 
                    ...
                    <div className="zoom-container" ref={zoomContainerRef}>
                      {showZoomMenu && (
                        <div className="timer-dropdown glass-premium">
                          {APP_CONFIG.ZOOM_LEVELS.map(level => (
                            <button 
                              key={level} 
